...
QJsonObject UIIntrospector::describe(QWidget* widget) {
  ...
  // Accessibility and hint information (useful for LLM context)
  ...
  if (!statusTip.isEmpty()) {
    json["statusTip"] = statusTip;
  }

  QString whatsThis = widget->whatsThis();
  if (!whatsThis.isEmpty()) {
    json["whatsThis"] = whatsThis;
  }

  QString accessibleName = widget->accessibleName();
  if (!accessibleName.isEmpty()) {
    json["accessibleName"] = accessibleName;
  }

  QString accessibleDesc = widget->accessibleDescription();
  if (!accessibleDesc.isEmpty()) {
    json["accessibleDescription"] = accessibleDesc;
  }
...
QJsonObject UIIntrospector::widgetToJson(QWidget* widget, int depth, const TreeOptions& opts) {
  ...
  // Add hints/accessibility info (concise version for tree)
  QString tooltip = widget->toolTip();
  if (!tooltip.isEmpty()) {
    json["toolTip"] = tooltip;
  }
